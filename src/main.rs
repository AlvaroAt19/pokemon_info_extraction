...
impl Pokemon {
    fn new(name:String,weight: u16, type1: String, type2: String, hp:u16, attack:u16, defense:u16, special_attack:u16, special_defense:u16, speed:u16) ->Pokemon{
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for i in 1..=10 {
        let pkm: Pokemon = get_pokemon(i).unwrap();
        wtr.serialize(pkm)?;
    };
    wtr.flush()?;
    Ok(())
...
async fn get_pokemon(i:u8) -> Result<Pokemon, reqwest::Error>{
    //Create a Pokemon
    let mut pkm:Pokemon = Pokemon::new(String::from("None"),0,String::from("None"),String::from("None"),0,0,0,0,0,0);
    //Get API
    let content: String = reqwest::get(format!("https://pokeapi.co/api/v2/pokemon/{}/",i))
        .await?
        .text()
        .await?;
    //Convert the responte to Json
    ...
    pkm.type2 = match content["types"].as_array().unwrap().len(){
        2 => content["types"][1]["type"]["name"].as_str().unwrap().to_owned(),
        _ => String::from("None")
    };
